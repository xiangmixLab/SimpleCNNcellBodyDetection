function cen_all=get_centroid_from_bw_100721(PMapt,img1,channel_to_do,net2)
...
%                 cen_all(ctt,:)=(stats1(k).Centroid-[stats1(k).BoundingBox(3)/2,stats1(k).BoundingBox(4)/2])+stats(j).Centroid;
            cen_all(ctt,:)=stats1(k).Centroid+stats1(k).BoundingBox(1:2)+stats(all_pt_classify_idx(j)).BoundingBox(1:2)+[-4,-8]; % replace the local position to global position in the slice            
            ctt=ctt+1;
        end
    end
end
