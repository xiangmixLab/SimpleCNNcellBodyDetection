function [score_result]=neuron_PMap_auto_ResNet101_mono_072121(net,img,major_slice_area,patch_size)
...
%         patch_t=imresize(patch_t,[36,36]);
        ...
        patch_t(:,:,2)=patch_t(:,:,2)/max(max(patch_t(:,:,2)))*0;
        patch_t(:,:,3)=patch_t(:,:,3)/max(max(patch_t(:,:,3)))*0;
